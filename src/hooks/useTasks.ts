...
export function useTasks(): UseTasksState {
    ...
    return (Array.isArray(input) ? input : [])
      .filter((t) => {
        ...
        return (
          ...
          t.title.length > 0 &&
          Number.isFinite(Number(t.revenue)) &&
          Number.isFinite(Number(t.timeTaken)) &&
          Number(t.timeTaken) > 0
        );
      })
      .map((t, idx) => {
  ...
  // BUG 1: Single fetch with ref guard to prevent double fetch
  useEffect(() => {
  ...
  // REMOVED: The second useEffect that caused duplicate fetches (BUG 1 fix)
  ...
  //  BUG 2: Add clearLastDeleted function to reset state when snackbar closes
